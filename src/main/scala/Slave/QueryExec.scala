...
object QueryExec extends Serializable {
  ...
  def KNNQuery(RDDRootNode : RDD[(InternalNode, Int)], Arrayrefe :Array[(Array[ReferenceINFO],Int)],
               Query : String, MapParttion : Map[Int,Int], InitRange : Double,
               K : Int, Path : String):(Array[(String,Double)],Int,Int,Double)={
    var querystarttime = System.currentTimeMillis()
    var query = Query
  ...
  def ANNQuery(RDDRootNode : RDD[(InternalNode, Int)], Arrayrefe :Array[(Array[ReferenceINFO],Int)], Query : String, MapParttion : Map[Int,Int],
               InitRange : Double, K : Int, Path : String):(Array[(String,Double)],Double,Int)={
    var querystarttime = System.currentTimeMillis()
    var query = Query
...
class ANNQuery(RootNode: InternalNode, partition: Int, ReferencepointArr: Array[(Array[ReferenceINFO],Int)],
               querydata:String, MapPartition: Map[Int,Int], InitRange: Double, Path : String, k:Int) {
...
//  println(s"PartitionID: $partition K: $k Range: $range MaxRan: $MaxDist dpo: $dpo Query: $querydata")
  import scala.tools.nsc.io.File

  val resString = s"$querydata $k ${range.toInt} ${dpo.toInt}\n"
  //println(resString)
  File("/root/spark-3.1.2-bin-hadoop3.2/data/lhb/Result.txt").appendAll(resString)
